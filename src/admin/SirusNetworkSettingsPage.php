...
final class SirusNetworkSettingsPage
{
    ...
    public static function getDefaultAccess(): array
    {
        ...
        if (! is_array($raw)) {
            return ['enabled' => false, 'roles' => ['administrator']];
        }
    ...
    public function handle_save(): void
    {
        ...
        update_site_option(
            self::OPTION_DEFAULT,
            ['enabled' => $default_enabled, 'roles' => $default_roles]
        );
        ...
        foreach ($raw_overrides as $blog_id => $site_data) {
            ...
            $overrides[$blog_id] = ['enabled' => $enabled, 'roles' => $roles_sanitized];
        }
        ...
        wp_safe_redirect(
            add_query_arg(
                ['page' => 'sirus-network-settings', 'updated' => '1'],
