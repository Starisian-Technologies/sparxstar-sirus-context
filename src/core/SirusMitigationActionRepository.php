...
final readonly class SirusMitigationActionRepository
{
    ...
    public function expireAction(int $actionId): void
    {
        ...
        $this->wpdb->query(
            $this->wpdb->prepare(
                sprintf("UPDATE %s SET status = 'expired', expires_at = %%d WHERE id = %%d", $table), // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is an internal $wpdb->prefix identifier, not user input
                time(),
                $actionId
            )
        );
