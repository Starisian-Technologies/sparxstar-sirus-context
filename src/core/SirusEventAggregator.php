...
final readonly class SirusEventAggregator
{
    ...
    public function prune(int $days = 7): void
    {
        ...
        $this->wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
            $this->wpdb->prepare(
                sprintf('DELETE FROM %s WHERE bucket_start < %%d', $table), // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is an internal $wpdb->prefix identifier, not user input
                $threshold
            )
        );
    }
