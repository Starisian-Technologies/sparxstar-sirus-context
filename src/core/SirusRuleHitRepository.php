...
final readonly class SirusRuleHitRepository
{
    ...
    public function incrementHit(string $ruleKey, string $deviceId = '', string $sessionId = ''): void
    {
        ...
        if (is_array($existing) && isset($existing['id'])) {
            ...
            $this->wpdb->query(
                $this->wpdb->prepare(
                    "UPDATE `{$table}` SET hit_count = hit_count + 1, updated_at = %d WHERE id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is an internal $wpdb->prefix identifier, not user input
                    time(),
                    (int) $existing['id']
                )
            );
    ...
    public function getRecentHits(int $limit = 100): array
    {
        ...
        $sql = $this->wpdb->prepare(
            "SELECT * FROM `{$table}` ORDER BY created_at DESC LIMIT %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is an internal $wpdb->prefix identifier, not user input
            $limit
        );
